...
class ModelPredictiveRL(Policy):
    ...
    def configure(self, config, device):
        ...
        if self.linear_state_predictor:
            ...
            self.value_estimator = ValueEstimator(config, graph_model, self.exploration_alg)
            ...
            if self.share_graph_model:
                ...
                self.value_estimator = ValueEstimator(config, graph_model1, self.exploration_alg)
    ...
    def set_device(self, device):
        ...
        for model in self.model:
            new_model.append(model.to(device))
        self.model = new_model

    def set_epsilon(self, epsilon):
        if self.exploration_alg == "dropout":
            self.epsilon = 0.0
            self.value_estimator.value_network.set_dropout(epsilon)
        else:
